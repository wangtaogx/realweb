    ...
    public void start() {
    	try {
	        ...
	        if(StringUtil.isEmpty(realWebHome) || !homeDir.exists()){
	        	System.out.println("Home directory " + realWebHome + " does not exist. ");
	        	return ;
	        }
	        ...
	        PropertyConfigurator.configure(configDir.getAbsolutePath()+"/log4j.properties");
	        RealWebClassLoader loader = new RealWebClassLoader(parent, libDir);
	        loader.addDir(configDir);
	        loader.addDir(resourcesDir);
	        Thread.currentThread().setContextClassLoader(loader);
	        Class<?> containerClass = loader.loadClass("com.tao.realweb.container.RealWebServer");
	        Object object = containerClass.newInstance();
	        if(object instanceof RealWebServer){
	        	server = (RealWebServer)object;
	        	RealWebConfig realWebConfig = RealWebConfig.getInstance(configDir.getAbsolutePath()+"/application.xml");
	        	String pluginDirString = realWebConfig.getString("realweb.plugin.dir");
	        	if(StringUtil.isEmpty(pluginDirString)){
	        		realWebConfig.putObject("realweb.plugin.dir", pluginDir.getAbsolutePath());
	        	}
